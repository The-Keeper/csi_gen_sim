...
const per_report_output = $derived( 
    data.reports.map( (report, report_idx) => {
		...
		const criterion_calc_by_subj = layout.criteria.map((crit, crit_idx) => {
			return {
				subjects: reports_input[report_idx].subjects.map((subject, subject_idx) => {
		...
		const total_by_subject = reports_input[report_idx].subjects.map((subj, subj_idx) => {
		...
		const min_max_crit_by_subj = reports_input[report_idx].subjects.map((subj, subj_idx) => {
